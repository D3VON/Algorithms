...
STL, vectors, priority_queues (min and max): passing back and forth between
min and max priority_queues is a relatively huge inefficiency here.  But I 
used them in order to insure error free heapifying -- my heap is not guaranteed
to work right, and there comes a point when you JUST WANT IT TO WORK RIGHT. 
...
void kill_min(priority_queue<vertex, vector<vertex>, less<vertex> > max)
{
  ...
  while ( !min.empty() && V[min.top().index].incidents < 2)
  {
   ...
   {
    min.pop(); // if it is already dead, pop it out.
   }
   else
   {
