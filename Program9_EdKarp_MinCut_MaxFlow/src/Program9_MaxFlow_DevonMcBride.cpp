...
int BFS(int num_v, int num_e, vertex V[], vertex s) 
{
      ...
      {           
         ...
         //         1  ==   1
          if (num_v-1 == tempE->v)
          {
            ...
            return do_path(V, 0, num_v - 1, num_v - 1); 
          }
...
bool Edmonds_Karp(int num_v, int num_e) 
{
	int tempE =0;
  if (V[0].edges != NULL)
  {
    //printf("Vertex zero: %d\n", V[0].index);
    while ( BFS(num_v, num_e, V, V[0]) ) 
    {
      //#######################################################################
...
int max_flow(edge *E, int num_e, int lastVertex)
{
  int maxflow = 0;
...
int main(int argc, char *argv[]) {
  ...
  */
	...
	if ( Edmonds_Karp(num_v, num_e) )
  {
    int MaxFlow = max_flow(E, num_e, num_v-1);
    printf("%d\n", MaxFlow);
  }else{
    printf("%d\n", 0);
  }
